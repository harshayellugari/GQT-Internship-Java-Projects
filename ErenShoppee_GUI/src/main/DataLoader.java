...
public class DataLoader {
    ...
    public static Category[] loadCategories() {
        ...
        for (int i = 0; i < 10; i++) {
            accessories.products[i] = new Product(i+1,"Electronic Accessory "+(i+1),"Generic","Useful electronic accessory",999+(i*200),50);
        }
        SubCategory tablets = new SubCategory("Tablets");
        for (int i = 0; i < 10; i++) {
            tablets.products[i] = new Product(i+1,"Tablet Model "+(i+1),"BrandTab","10 inch display tablet",19999+(i*1500),25);
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 10; j++) {
                fashion.subCategories[i].products[j] = new Product(j+1,"Clothing Item "+(j+1),"BrandWear","Comfortable daily wear",999+j*300,40);
            }
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 10; j++) {
                home.subCategories[i].products[j] = new Product(j+1,"Home Product "+(j+1),"HomeBrand","Essential home utility",1499+j*500,35);
            }
        }
        categories[2] = home;
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 10; j++) {
                books.subCategories[i].products[j] = new Product(j+1,"Book Title "+(j+1),"Author Name","Popular bestselling book",399+j*100,100);
            }
        }
        categories[3] = books;
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j = 0; j < 10; j++) {
                sports.subCategories[i].products[j] = new Product(j+1,"Sports Item "+(j+1),"FitBrand","Professional sports equipment",1999+j*700,20);
            }
        }
        categories[4] = sports;
